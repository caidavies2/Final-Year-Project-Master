...
char deviceName[] = "node3"; // set a unique name for each device connected to the broker
...
void setup() {
  ...
  else
  {
    ...
    // Make a HTTP request:
    ethClient.println("GET /~hivenode/fyp/count.php?node=3");
...
void mainLoop()
{
  ...
  if(oldState != buttonState && buttonState == 1)
  {
    if(capturedStatus == true)
    {
      ...
      client.publish("public/cai-fyp/node-3/status", "saved");
...
//  Serial.println(pullyPreviousState);
    ...
    if(pullyState != pullyPreviousState)
  {
      cTouch(0,0,0);
      numberOfItems = 0;
      client.publish("public/cai-fyp/print", "print-node-3");
  }
